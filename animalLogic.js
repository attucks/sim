...
// - Stuck detection with a perpendicular nudge.
// - Food-seeking prioritized: higher speed + no dampening while eating.
// - Wander pattern: vertical half-step → horizontal half-step → repeat.
...
const AVOID_RADIUS = 5;             // personal space radius
...
// Alliances (stable keys)
const __alliances = new Map(); // "r|g|b" -> Set("r|g|b")
...
function formAlliance(familyA, familyB) {
  ...
  // Optional external mirror if your UI reads it
...
function maybeFormAlliance(a) {
  const potentialAllies = get("animal").filter(o =>
    o !== a && !areRelatives(a, o) && a.pos.dist(o.pos) < 180 && o.alive
  );
...
// Friendly relationship (fixes allied friendly-fire)
function isFriendly(a, b) {
  return areRelatives(a, b) || areAlliedColors(a.familyColor, b.familyColor);
}
...
function findClosestEnemy(a) {
  const enemies = get("animal").filter(o => o !== a && !isFriendly(a, o) && o.alive);
...
function findTarget(a) {
  ...
  for (const f of foods) {
    const d = a.pos.dist(f.pos);
    if (d < foodDist) { closestFood = f; foodDist = d; }
  }

  for (const o of others) {
    if (isFriendly(a, o)) continue;
    const d = a.pos.dist(o.pos);
    if (d < preyDist) { closestPrey = o; preyDist = d; }
  }
...
function clampToPen(x, y) {
  ...
  return vec2(clamp(x, left, right), clamp(y, top, bot));
}
...
function ensureWanderTarget(a) {
  if (!a._wanderAxis) a._wanderAxis = "vertical";
  if (!a._wanderTarget) {
    a._wanderTarget = a._wanderAxis === "vertical"
...
function tryMove(a, dirVec, baseSpeed, opts = {}) {
  ...
  // Barrier intersection test
  const blocked = get("barrier").some(b => {
    const p = b.pos;
    const w = b.width || 10, h = b.height || 10;
    return (
      nextPos.x < p.x + w &&
      nextPos.x + a.width > p.x &&
      nextPos.y < p.y + h &&
      nextPos.y + a.height > p.y
    );
  });

  if (!blocked) {
    // Personal space avoidance
    const buddy = get("animal").find(o => o !== a && o.alive && o.pos.dist(nextPos) < AVOID_RADIUS);
    if (buddy) {
...
function decideMission(a) {
  ...
  if (a.hunger > (3 - a.greed)) {
    const foods = get("food");
    const food = foods.length ? foods.reduce((c, f) => a.pos.dist(f.pos) < a.pos.dist(c.pos) ? f : c) : null;
    if (food) { a.mission = { type: "eat", target: food, timer: 0 }; return; }
  }

  // 3) Defend nearby friendly legacy (own or allied)
  if (get("legacy").length > 0 && a.enemies && a.enemies.length > 0) {
    const enemyNearLegacy = a.enemies.find(e =>
      get("legacy").some(l =>
        (colorsMatch(l.familyColor, a.familyColor) || areAlliedColors(a.familyColor, l.familyColor)) &&
        l.pos.dist(e.pos) < 60
      )
    );
    if (enemyNearLegacy) { a.mission = { type: "defend", target: enemyNearLegacy, timer: 0 }; return; }
  }
  ...
  if (a.territorial > 0.5 && get("legacy").length > 0 && rand(1) < 0.4) {
    ...
    if (myLegacy) { a.mission = { type: "patrol", target: myLegacy, timer: 0 }; return; }
  }

  // 5) Roam with pack (friendly nearby)
  if (a.allies && a.allies.length > 0 && rand(1) < 0.3) {
    const mate = a.allies[Math.floor(rand(0, a.allies.length))];
    if (mate) { a.mission = { type: "roam", target: mate, timer: 0 }; return; }
  }

  // 6) Attack enemy if available
  if (a.enemies && a.enemies.length > 0) {
    const nearEnemy = a.enemies.find(e => a.pos.dist(e.pos) < 180);
    if (nearEnemy && rand(1) < 0.7) { a.mission = { type: "attack", target: nearEnemy, timer: 0 }; return; }
  }
...
function ensureStats(animal) {
  if (!animal.stats) animal.stats = { kills: 0, foods: 0, lifetime: 0 };
...
onUpdate("animal", (a) => {
  ...
  // periodic scans + hungry retarget
  a.scanTimer = (a.scanTimer || 0) + dt();
  if (a.scanTimer > 0.5) {
    a.allies  = get("animal").filter(o => o !== a && isFriendly(a, o) && o.alive);
    a.enemies = get("animal").filter(o => o !== a && !isFriendly(a, o) && o.alive);

    if (a.hunger > (3 - a.greed)) {
      const foods = get("food");
      if (foods.length) {
        const nearestFood = foods.reduce((c, f) => (a.pos.dist(f.pos) < a.pos.dist(c.pos) ? f : c), foods[0]);
  ...
  if (a.hunger > starvationThreshold) {
    a.hungerTime = (a.hungerTime || 0) + dt();
  ...
  switch (a.mission.type) {
    ...
    case "attack": {
      if (a.mission.target) {
        // disengage if friendly now (alliance formed)
        if (isFriendly(a, a.mission.target)) {
    ...
    default: {
      ...
      const multKey = a.mission.type === "explore" ? "explore" : "default";
      const s = animalSpeed * (SPEED_MULT[multKey] ?? 1);
      if (a._wanderTarget) {
        if (a.pos.dist(a._wanderTarget) <= WANDER_ARRIVE_EPS) {
  ...
  if (a.mission?.type === "attack" && a.mission.target) {
    ...
    // drop if target died or became friendly
    if (!t.alive || isFriendly(a, t)) {
      a.mission = { type: "none", target: null, timer: 0 };
    } else if (a.pos.dist(t.pos) < 5) {
      if (a.attackTimer >= a.attackCooldown) {
        ...
        if (rand(1) > 0.1) {
          ...
          if (t.health <= 0 && t.alive !== false) {
            ...
            try { killAnimal(t, { cause: "defeated", killer: a }); }
            catch { killAnimal(t, "defeated"); }

            a.mission = { type: "none", target: null, timer: 0 };
          }
        } else {
          addNews?.(`${fullName(a)} missed ${t.firstName}!`);
        }
      }
    }
  }
  ...
  const colorMultiplier =
    ...
    (a.mission.type === "flee") ? 0.7 : 1.0;
  a.color = scaleColor(a.familyColor, colorMultiplier);

  // === Badge for gold age ====================================================
  if (a.stats.lifetime > goldAge && !a.hasBadge) {
    a.badge = add([rect(5, 5), pos(a.pos.x, a.pos.y - 8), color(a.familyColor), area(), "badge"]);
    a.hasBadge = true;
  }
  if (a.badge) a.badge.pos = vec2(a.pos.x, a.pos.y - 8);
  ...
  if (a.helpTimer > 1) {
    if (a.allies) {
      for (const o of a.allies) {
        if (o.mode === "flee" && o.target && !isFriendly(a, o.target)) {
  ...
  // === Birth logic ===========================================================
  if (a.hunger < 1) {
    a.satedTime = (a.satedTime || 0) + dt();
    if (a.satedTime > birthingTime) a.readyToBirth = true;
  } else {
    a.satedTime = 0;
  }
  ...
  if (moved > STUCK_DIST_EPS) a._stuckTimer = 0;

  // apply ongoing nudge if active
  if (a._nudgeEnd && time() < a._nudgeEnd && a._nudgeVec) {
    tryMove(a, a._nudgeVec, NUDGE_SPEED);
  } else {
    a._nudgeEnd = null;
    a._nudgeVec = null;
  }
  ...
  if (a._stuckTimer > STUCK_TIME_LIMIT) {
    ...
    // Perpendicular nudge
    a._nudgeVec = vec2(-v.y, v.x).unit();
    a._nudgeEnd = time() + NUDGE_TIME;

    // Reset wander leg to avoid oscillation
    a._wanderTarget = null;
  }
