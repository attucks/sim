...
// - Allies DO NOT destroy each other's legacies (mushrooms).
...
// ===== Color utils (shared by scaling + alliance keys) =======================
...
function colorKey(c) {
  ...
  // Round for stability if Kaboom stores floats
...
// ===== Alliances (fixed: stable keys + allied check) =========================
...
function formAlliance(familyA, familyB) {
  ...
  // If an external store exists, mirror with string keys for compatibility
...
function maybeFormAlliance(a) {
  ...
  for (const o of potentialAllies) {
    if (traitsCompatible(a, o)) {
      // Safe: stores by stable string keys
  ...
  if (a.repelTimer > 0.5) {
    for (const b of [...get("barrier"), ...get("legacy")]) {
      ...
      if (
        ...
        !areAlliedColors(a.familyColor, b.familyColor) // NEW: skip allies
      ) {
